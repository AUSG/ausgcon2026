...
export function encodeStrokes(strokes: Stroke[]) {
  ...
  for (const stroke of strokes) {
    ...
    if (encodedPoint) coordinates.push(STROKE_SEPARATOR, STROKE_SEPARATOR);
  }
...
export function decodeStrokes(encoded: string) {
  ...
  if (bytes.byteLength === 0) return { pointCount: 0, strokes: [] };
  ...
  for (let offset = 0; offset < bytes.byteLength; offset += 4) {
    ...
    points.push({ x: x / COORDINATE_MAX, y: y / COORDINATE_MAX });
    pointCount += 1;
    if (pointCount > MAX_POINT_COUNT) {
